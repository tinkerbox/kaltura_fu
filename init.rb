...
#KalturaFu.config = YAML.load_file(kaltura_yml).recursively_symbolize_keys[:development]
...
unless[:partner_id,:subpartner_id,:administrator_secret].all? {|key| KalturaFu.config.key?(key)}
  raise RuntimeError, "Kaltura config requires :partner_id, :subpartner_id, and :administrator_secret keys"
end
